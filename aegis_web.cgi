...
END {
  ...
  min=(NR>'$_MAX')?NR-'$_MAX':0;while(--c>min && uts[c]>'$_ST'){
    ...
    print "<p class=\"new "dir[f[7]] itf[f[5]] adt[dst[1]]"\"><log-ts>"strftime("%F %T",f[2])"</log-ts><log-lbl></log-lbl><log-dir></log-dir>"protoname(f[4])"<log-rll><log-if></log-if></log-rll><log-rem><log-rip>"rem[1]"</log-rip>"rpt"</log-rem><log-lll><log-lnm>"lnm"</log-lnm></log-lll><log-loc><log-lip>"loc[1]"</log-lip>"lpt"</log-loc></p>"
  }
}' $_LF
}
