...
_getLog() {
  ...
  /bin/grep -F $_KEY /var/log/log-message | /usr/bin/tail -n$_MAX | { IFS=;while read -r LINE; do
    ...
    case $LINE in
      *"IN=$_WIF"*)
        _LOG="<p class='new incoming wan'>$_PT Blocked <log-if>WAN</log-if> <log-dir>incoming</log-dir> $_PROTO packet from remote IP <log-rip onclick=\"getIpInfo('$_SRC');\">$_SRC</log-rip>$_SPT to local IP <log-lip>$_DST</log-lip>$_DPT</p>$_LOG"
        ;;
      *"OUT=$_WIF"*)
        _LOG="<p class='new outgoing wan'>$_PT Blocked <log-if>WAN</log-if> <log-dir>outgoing</log-dir> $_PROTO packet to remote IP <log-rip onclick=\"getIpInfo('$_DST');\">$_DST</log-rip>$_DPT from local IP <log-lip>$_SRC</log-lip>$_SPT</p>$_LOG"
        ;;
      *"IN=$_TIF"*)
        _LOG="<p class='new incoming vpn'>$_PT Blocked <log-if>VPN</log-if> <log-dir>incoming</log-dir> $_PROTO packet from remote IP <log-rip onclick=\"getIpInfo('$_SRC');\">$_SRC</log-rip>$_SPT to local IP <log-lip>$_DST</log-lip>$_DPT</p>$_LOG"
        ;;
      *"OUT=$_TIF"*)
        _LOG="<p class='new outgoing vpn'>$_PT Blocked <log-if>VPN</log-if> <log-dir>outgoing</log-dir> $_PROTO packet to remote IP <log-rip onclick=\"getIpInfo('$_DST');\">$_DST</log-rip>$_DPT from local IP <log-lip>$_SRC</log-lip>$_SPT</p>$_LOG"
        ;;
    esac
    _LINE=$LINE
  done
